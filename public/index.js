...
const drawFive = () => {
  axios.get("http://18.220.86.46/api/robots/shuffled").then((res) => {
...
const duel = () => {
  ...
  setTimeout(() => {
    axios.post("http://18.220.86.46/api/duel", { compDuo, playerDuo }).then(({ data }) => {
...
const getPlayerStats = () => {
  axios.get("http://18.220.86.46/api/player").then(({ data: { wins, losses } }) => {
...
const getAllBots = () => {
  axios.get("http://18.220.86.46/api/robots").then(({ data }) => {
